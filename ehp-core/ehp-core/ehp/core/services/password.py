...
from ehp.core.models.param import AuthenticationParam, PasswordParam
...
@router.post("/user/pwd", response_class=JSONResponse)
    ...
    try:
        ...
        async with db_manager.transaction() as db_session:
            ...
            if auth:
                ...
                auth.reset_code = generate_random_code(4)
                await db_session.flush()

                send_notification(
                    "Password Reset - CODE",
                    f"Your password reset code is: {auth.reset_code}",
                    auth.user_email,
                    "",
                    auth.id,
                )
